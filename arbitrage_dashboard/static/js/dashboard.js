...
async function loadUsersAdmin(){
  ...
  r.users.forEach(x=>{
    ...
    label.textContent=`${x.username}${x.is_admin?' (admin)':''}${tgInfo} ${x.subscription_approved?'✅':'⏳'}`;
    label.style.cssText='flex:1;min-width:140px;';
    row.appendChild(label);
    if(!x.is_admin){
      const btnSub=document.createElement('button'); btnSub.className='btn'; btnSub.style.padding='4px 10px'; btnSub.style.fontSize='12px';
      btnSub.textContent=x.subscription_approved?t.disableSub:t.approveSub;
      btnSub.onclick=async()=>{btnSub.disabled=true; await apiPost('/api/admin/subscription',{username:x.username,approved:!x.subscription_approved}); await loadUsersAdmin();};
      row.appendChild(btnSub);
      const btnDel=document.createElement('button'); btnDel.className='btn btn-danger'; btnDel.style.padding='4px 10px'; btnDel.style.fontSize='12px';
